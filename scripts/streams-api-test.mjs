...
function startServer() {
  return spawn(process.execPath, ['node_modules/tsx/dist/cli.mjs', 'server/index.ts'], {
    // ADMIN_PASSWORD:'' pins the open (ungated) state regardless of .env.local,
    // which may set a password for local admin-login testing.
    env: { ...process.env, PORT: String(PORT), SIM_MODE: '1', STREAMS_CONFIG_PATH: streamsFile, NODE_ENV: 'development', ADMIN_PASSWORD: '' },
    stdio: 'ignore',
  });
}
