// Fetches one profile picture per host at boot. Priority: Twitch Helix (app
// keys), then the official Kick API (app keys; Cloudflare-proof), then Kick's
// keyless web endpoint (often 403s server-side, kept as last resort).
// Fail-soft everywhere: any failure just leaves the client's letter fallback.
// Boot-only by design — a server restart picks up new pictures. (If live
// refresh is ever wanted, re-run this on a long timer and rebroadcast.)
...
export async function fetchHostAvatars(config: AppConfig): Promise<HostAvatars> {
        ...
        for (const u of data.data || []) {
          ...
          for (const host of HOSTS) {
            if (login === config.twitchChannels[host].toLowerCase() && u.profile_image_url) {
              avatars[host] = u.profile_image_url;
            }
          }
        }
      }
  ...
  if (config.kick.configured && HOSTS.some((h) => !avatars[h])) {
    try {
      ...
      if (token) {
        ...
        if (idToHost.size > 0) {
          ...
          for (const u of users.data || []) {
            ...
            if (host && u.profile_picture) avatars[host] = u.profile_picture;
          }
        }
      }
    } catch (err) {
  ...
  // ── Kick web endpoint, last resort per host still missing (no keys needed) ──
  for (const host of HOSTS) {
    if (avatars[host]) continue;
    try {
      ...
      if (d.user?.profile_pic) avatars[host] = d.user.profile_pic;
    } catch (err) {
